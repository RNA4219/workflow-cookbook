...
exit 1は不正入力、衝突、破損、I/O失敗。JSON出力で運用準拠は`not_evaluated`とする。

未知の引数や必須引数の不足など、CLI構文エラーもJSONとexit 1で返す。
`--help`は利用案内を表示してexit 0で終了する。

## 保全と整合性

- コピー元と導入先の包含関係を拒否し、自己コピーを防ぐ。
- 大小文字を区別しない環境で衝突するパス成分やファイル・ディレクトリの組合せは、準備前に拒否する。
...
- 既存AGENTSはO_APPENDで追記し、全体置換や元の内容への書き戻しは行わない。
  新規AGENTSは排他的に作成し、要求するmodeは0600とする（既存ファイルの権限は保全）。
  追記前後の内容・実体の変化、短い書込は要対応として返す。
...
Gitへ共有するときのmodeは`--check-git-modes`で別途検査する。
この検査はGitを必要とし、indexのstage 0とmanifestの全ファイルのmodeを照合する。
通常の`--check`は従来どおりGit不要。どちらもindexや設定を書き換えない。
既存v1/v2のADOPTION本文とmanifest形式は変更しない。
