...
- `schemas/self-improvement/v1/`: Gate観測とeffectiveness reportの契約正本。
- `docs/tasks/task-self-improvement-gate-effectiveness-20260712.md`: review-only Gate解析のTaskとAcceptance導線。
...
3. `docs/birdseye/index.json.generated_at` と `docs/birdseye/hot.json.generated_at` が同じ更新サイクルの 5 桁ゼロ埋め世代番号へ進んでいるか確認します。
...
6. `python tools/ci/check_birdseye_freshness.py --check` を実行し、
   `generated_at`、`mtime`、caps 参照の整合が取れていることを確認します。
   ホットリストの確認日も見たい場合は
   `--max-verified-age-days 90` のように freshness window を追加します。
7. `GUARDRAILS.md` の [鮮度管理](../GUARDRAILS.md#鮮度管理staleness-handling) に従って、必要に応じて人間へ再生成依頼またはインシデント共有を行います。
