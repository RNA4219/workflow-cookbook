...
| gate_effectiveness_analyzer | `GateEffectivenessReport`、review-only `PeriodicNudge` | shipyard-cpの`ImprovementObservationBundle`、閾値設定、任意の前回report | 自己改善契約と集計規則の正本。actionを4値に限定し、Gate設定を変更しない。schemaは`schemas/self-improvement/v1/`。 |
