...
| 仕様 | [docs/spec.md](spec.md) | Birdseye・参照実装・CI / Governance を含む公開インターフェースと互換条件を集約。 | CLI や外部契約を変更する際に `TASK.codex.md` の[Task Seed Template](../TASK.codex.md#task-seed-template)と整合性を確認。 |
| 設計 | [docs/design.md](design.md) | ドキュメント、Birdseye、参照実装、CI / Governance の構成と制御フローを整理。 | 設計レビューで `CHECKLISTS.md` の[Release](../CHECKLISTS.md#release)項目と照合。 |
...
- **改善 backlog の継続棚卸し**
  - [docs/addenda/N_Improvement_Backlog.md](addenda/N_Improvement_Backlog.md) を正本 backlog として扱い、Security / Release / Acceptance / Cross-Repo plugin の改善候補を継続的に整理する。
  - 実装前は backlog ID を Task Seed や acceptance record に記載し、実装後は `CHANGELOG.md` と関連 docs へ反映する。
  - 中長期の拡張候補は [docs/addenda/P_Expansion_Candidates.md](addenda/P_Expansion_Candidates.md)
    で管理し、直近実装候補へ降ろすときに `N_Improvement_Backlog.md` へ移す。
...
- [docs/addenda/P_Expansion_Candidates.md](addenda/P_Expansion_Candidates.md)：中長期の拡張候補と設計メモを整理する docs。**利用シーン**：直近 backlog へ載せる前のアイデア整理や repo 横断の拡張方針を検討するときに参照。
