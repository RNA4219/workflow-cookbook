...
`shipyard-cp`のAudit-backed producerから受け取ったbundleをschema validationし、`tools/ci/gate_effectiveness.py`が使い捨て集計を行う。reportとnudgeはproposal projectionであり、Gate所有repoの設定を直接変更しない。責務lookupは`governance/repo-responsibilities.yaml`だけを参照し、Agent_tools側はリンクのみを保持する。
