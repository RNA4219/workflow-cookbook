...
3. 対象ruleと実コード・設定を照合し、移動／削除時は参照を更新する。CI自動照合は将来実装予定であり、現時点ではレビューで行う。
   期限切れを日付更新だけで再承認せず、緩和策と残留リスクを責任者が再評価する。

## 例外台帳

以下の行番号は登録時点の記録。期限経過・移動した参照は承認継続の証明に使用せず、現在のシンボル／設定と照合して再評価する。
本方針改修では既存例外の期限延長・新規承認を行っていない。
